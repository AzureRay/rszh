...
public class MoreRecordDao extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(delString != null){
			...
			atd.delMoreByOnce(idList, tableName);
			//重新获取
			//获取数据库表中的记录
			...
			List<CommentObject> rowList = atd.getRowNameList(tableName);
			request.setAttribute("rowList",rowList );
			request.getRequestDispatcher("/jsp/gaojichaxun/modMoreSelectRow.jsp").forward(request, response);
		}
	}
}
