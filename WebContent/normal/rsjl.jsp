...
<body>
	...
	<a style="font-size:18px;" href="/normalJlszServletRequest?tableName=<%=tableNameList.get(i).getValues().get("table_name")%>&current=1"><%=tableNameList.get(i).getValues().get("table_name")%></a>&nbsp;&nbsp;
	...
	<form id="table_tool"
		...
		<form method="post" action="/normalJlszServletRequest?current=1">
			<select name="人员类别">
			<%
		...
		<div class="dtree">
			...
			%>
			<script type="text/javascript">
				d.add(<%=id%>,<%=parentId%>,"<%=name%>", '/normalJlszServletRequest?部门=<%=name%>&人员类别=<%=rylb%>');
			</script>
			<%
				}
			%>
