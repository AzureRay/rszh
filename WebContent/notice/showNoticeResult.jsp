...
<body>
<%
	SaveNotice sc = new SaveNotice();
	List<CommentObject> list = sc.getNoticeRecord();
%>
	<div id="contentRight" style="width: 100%; height: 440px; font-size: 14px; margin-left: 0px; margin-bottom: 0px; border: 1px solid #DDDDDD; overflow: scroll;">
		<form action="/delNoticeRecord" id="">
			<a href="/notice/selectTable.jsp" id="add" class="btn btn-success">添加</a>
			...
			for(int i=0;i<list.size();i++){
				%><tr><td><%= i+1 %></td>
				<td><input type="checkbox" name="<%= "name"+i %>" id="noticeId" value="<%= list.get(i).getValues().get("id")%>"></td>
				<td><%= list.get(i).getValues().get("notice_content") %></td>
				<td><%= list.get(i).getValues().get("notice_num") %></td>
				<td><%= list.get(i).getValues().get("notice_time") %></td></tr><%
			}
			%>
			</table>
		</form>
