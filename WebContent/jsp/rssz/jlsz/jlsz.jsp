    ...
    <script type="text/javascript">
    	$(function(){
    		$('#add').click(function(){
    			...
    			window.location.href="/jsp/rssz/jlsz/addNewTable.jsp?current="+current;
    		 });
    		$('#mod').click(function(){
...
<body>
	...
	for (int i = 0; i < tableInfo.size(); i++) {
		String tableName = (String) tableInfo.get(i).getValues()
			...
			<td><%=tableName%></td>
			<td><a href="/showDetialsServlet?tableName=<%=tableName%>&current=1">查看详情</a></td>
			</tr>
			<%
	}
	%>	
	...
	<div class="inline pull-right page">
      	 ...
      	 &nbsp;<a href='/jlszServlet?current=1'>第一页</a>
      	 <%
      	 if(current > 1){
      		 %> &nbsp;<a href='/jlszServlet?current=<%= current - 1%>'>上一页</a><%
      	 }
      	 %>
      	 <%
      	 if(current < pageCount){
      		 %> &nbsp;<a href='/jlszServlet?current=<%= current + 1%>'>下一页</a><%
      	 }
      	 %>
         &nbsp;<a href='/jlszServlet?current=<%= pageCount %>' >最后一页</a> 
         <form action="/jlszServlet" method="post">
