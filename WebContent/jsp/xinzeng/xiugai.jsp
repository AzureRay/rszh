...
<div class="inline pull-right page">
      	   当前&nbsp;<%= location+1 %>/<%= pageCount %>&nbsp;页 
      	 <%
      	 if(location > 0){
      		 Object before = ids.get(location-1);
      		 
      		 %> &nbsp;<a href='/xiugaiPersonal?id=<%= before%>'>上一页</a><%
      	 }
      	 %>
      	 <%
      	 if(location < pageCount-1){
      		Object after = ids.get(location+1);
      		 %> &nbsp;<a href='/xiugaiPersonal?id=<%=after %>'>下一页</a><%
      	 }
      	 %>
    </div>
	<form class="form-inline definewidth m20" id="table_tool"
		action="/xiugaiCompleteServlet" method="post">
		...
		<center>
			<input type="submit" value="确定" class="btn btn-primary"> <input
				...
				<a href="/showPersonal?current=<%=currentPage %>" class="btn btn-primary">取消</a>
		</center>
	</form>
</body>
</html>