	...
	$(function() {
		...
		$("#photo").click(function() {
			...
			if (len == 0) {
				...
				var currentPage = $("#currentPage").val();
				$("#id").val(val);
				window.location.href="/jsp/personalPhoto/personPhoto.jsp?currentPage="+currentPage+"&id="+val;
			}
		});
	});
</script>
...
%>
	...
	<div class="inline pull-right page">
      	 ...
      	 &nbsp;<a href='/showPersonal?current=1'>第一页</a>
      	 <%
      	 if(current > 1){
      		 %> &nbsp;<a href='/showPersonal?current=<%= current - 1%>'>上一页</a><%
      	 }
      	 %>
      	 <%
      	 if(current < pageCount){
      		 %> &nbsp;<a href='/showPersonal?current=<%= current + 1%>'>下一页</a><%
      	 }
      	 %>
         &nbsp;<a href='/showPersonal?current=<%= pageCount %>' >最后一页</a> 
         <form action="/showPersonal" method="post">
